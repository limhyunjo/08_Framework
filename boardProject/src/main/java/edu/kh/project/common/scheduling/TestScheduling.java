...
public class TestScheduling {
	...
	//        cron = "0 50 12 7 5 3 2024" -> 5월 7일 화요일 마다 수행
...
//	@Scheduled(fixedRate = 5000) // ms 단위
	...
	// cron = "초 분 시 일 월 요일 [년도]"  (일요일 1 ~ 토요일 7)
	//@Scheduled(cron = "0/10 * * * * *") // 다른 거 다 상관 없이 초만 0초 == 매 0초마다 수행(1분 마다)
	//@Scheduled(cron = "0/10 * * * * *") // 0부터 10초마다 수행
//	@Scheduled(cron = "0 0 0 * * *") // 매일 자정마다 수행
//	@Scheduled(cron = "59 59 023 * * *") // 다음 날 되기 1초 전
