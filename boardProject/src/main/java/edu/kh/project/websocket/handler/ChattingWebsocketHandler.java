...
public class ChattingWebsocketHandler extends TextWebSocketHandler{
	...
	//                             ( + interceptor가 가로챈 클라이언트 세션이 담겨있음)
	
	// sessions : 연결된 클라이언트 (== /chatting 페이지에 접속한 사람)
	//               들의 접속 정보를 모두 모아두는 용도
	private Set<WebSocketSession> sessions
	 = Collections.synchronizedSet(new HashSet<>()); // 동기화된 HashSet 생성
	
	// 초기 설정 , 클라이언트와 연결이 완료되고 통신할 준비가 되면 실행됨
	@Override
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		
		// 연결된 클라이언트를 sessions에 추가
		sessions.add(session); // 접속한 사람을 sessions에 추가
