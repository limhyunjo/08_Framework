...
public class TestWebsocketHandler extends TextWebSocketHandler{ 
	// TextWebSocketHandler 인터페이스가 아닌 클래스임
	
	
	// WebSocketSession : 웹소켓 전용 세션
	// - 클라이언트 - 서버 간 전이중 통신을 담당하는 객체 (실시간으로 주고 받을 수 있게 함)
    // - SessionHandshakeInterceptor가 가로챈 세션 정보
	//    연결한 클라이언트의 HttpSession을 가지고 있음
	//    ( attributes에 추가한 값 ) 
	
	// Set : 중복 허용 안함 (데이터 구분할 수 있음)
	// synchronizedSet : 강제로 줄세운 동기화된 Set 생성
	// 동기화된 Set 생성
	...
	@Override
		...
		// 연결이 끝나면 명단에서 삭제해서 나가게 해줘야함
		
		// 웹소켓 연결이 끊긴 클라이언트 정보를
		// Set에서 제거 ( 현재 연결된 사람들의 정보만 있도록 )
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		//                      텍스트(내용)가 담겨있는 객체 
		// String으로 주고받지 못하고 TextMessage 객체 사용해야 함
		
		// Payload : 통신 시 탑재된 데이터
		log.info("전달 받은 메시지 : {}", message.getPayload());
		
		// 전달 받은 메시지를 
		// 현재 해당 웹소켓에 연결된 모든 클라이언트에게 전달할 수 있음
		for(WebSocketSession s : sessions) { 
			// sessions : 연결된 모든 클라이언트 정보가 담겨있음
...
----------------------------------------------------------------------------
 ...
 TextWebSocketHandler :  WebSocketHandler 인터페이스를 상속받아 구현한 텍스트 메세지 전용 웹소켓 핸들러 클래스
   ...
   BinaryWebSocketHandler : 파일 주고받는 웹소켓 
 */
