...
function getCompleteCount(){
  ...
  // fetch() : 비동기로 요청해서 결과 데이터 가져오기 ( 화면 깜빡임이 없음 )
...
const selectTodoList = () => { // 변수에 함수를 대입하는게 요즘 트랜드

 fetch("/ajax/selectList") // 비동기 요청해서 결과를 안의 주소로 가져오겠다는 코드

 .then( response => response.text()) // 응답 결과를 text로 변환

 .then(result =>{ // result == 첫번째 then에서 반환된 결과값

  console.log(result)
  console.log(typeof result) // String 나옴  JSON -String 
    ...
    const todoList = JSON.parse(result); // 배열로 바뀜
...
deleteBtn.addEventListener("click", () =>{
  ...
  fetch("/ajax/delete",{
    ...
    method : "DELETE",  // DELETE 방식 요청 -> @DeleteMapping 처리 (비동기 방식만 가능)
...
//-------------------------------------------------------------------------
// js 파일에 함수 호출 코드 작성 -> 페이지 로딩 시 바로 실행됨 (제일 밑에 놓기)
getCompleteCount();
getTotalCount();
selectTodoList();
